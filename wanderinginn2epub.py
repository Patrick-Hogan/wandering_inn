...
try:
    ...
    print(
        "Warning: pillow not found. New cover image will not be generated. Error:"
        f" {error}"
    )
...
class Chapter:
    ...
    def __init__(self, url, link_text="", volume=0, index=0):
        ...
        if self.name == "Glossary":
            ...
            self.index = 999999
        self.filename = (
            f"wandering_inn-{self.volume:02d}.{self.index:03d}-{self.name}.html"
        )
    ...
    def save(self, stream=sys.stdout, strip_color=False, image_path="./images"):
        ...
        author_note = [
            ...
            if p.getText()[0:6] == "Author" and "Note" in p.getText()[0:13]
        ]
        if author_note:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If true, remove color information from text (e.g. for display on"
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "Volume(s) to get for ebook (default: all; if chapter specified, default is"
            " None)"
        ),
    )
    parser.add_argument(
    ...
    output_group.add_argument(
        ...
        action="store_true",
        help=(
            "Pretty-print the effective index (list of chapters) and exit (useful for"
            " testing volume/chapter selection)"
        ),
    )
    output_group.add_argument(
        "--output-title",
...
def create_cover_image(base_image, title, subtitle=None, outdir=None):
    ...
    try:
        with Image.open(base_image) as image:
            ...
            editable.text(
                (pad, pad + title_font.getsize(title)[1]), title, color, font=title_font
            )
            if subtitle:
                subtitle_font = ImageFont.truetype(fontname, 30)
                height = (
                    pad
                    + title_font.getsize(title)[1]
                    + pad
                    + subtitle_font.getsize(subtitle)[1]
                )
...
def get_book(
    ...
    if chapter is not None:
        ...
        subtitle = (
            f"Volume {chapter.volume}.{chapter.index:03d} - Chapter {chapter.name}"
        )
    ...
    ebook_data["cover"] = create_cover_image(
        ebook_data["cover"], title, subtitle, outdir=cover_path
    )
    ...
    def include_chapter(ch):
        return (volume is None or ch.volume == volume) and (
            chapter is None or ch == chapter
        )

    for ch in tqdm(list(filter(include_chapter, index)), "Chapter"):
        filename = os.path.join(html_path, ch.filename)
        ebook_data["contents"].append(
            {"generate": False, "type": "text", "source": filename}
        )
...
def main():
        ...
        for chapter_title in args.chapter:
            if chapter_title == "latest":
                ...
                try:
                    ...
                    print(
                        f"Unable to find matching chapter for {chapter_title}: {error}"
                    )
    ...
    if args.by_chapter:
        for chapter in tqdm(index):
            ...
            gen.createEBookFile(
                os.path.join(args.build_dir, f'{chapter_data["title"]}.epub')
            )
    elif args.by_volume:
        for volume in tqdm(args.volume, "Volume"):
            ...
            gen.createEBookFile(
                os.path.join(args.build_dir, f'{volume_data["title"]}.epub')
            )
    else:
        get_book(
            ebook_data,
